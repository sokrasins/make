...
status_t dfu_init(void)
{
    // Register connection CB with net
    net_evt_handle = net_evt_cb_register(NET_EVT_CONNECT, net_evt_cb, NULL);
...
void dfu_task(void *params)
{
    ...
    net_evt_cb_deregister(net_evt_handle);
    ...
    vTaskDelete(dfu_task_handle);
}

void net_evt_cb(net_evt_t evt, void *ctx)
{
    // Run net task, ready to handle network state changes
    xTaskCreate(dfu_task, "DFU_Task", 8192, NULL, 2, &dfu_task_handle);
}